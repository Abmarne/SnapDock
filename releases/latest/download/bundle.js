(() => {
  ...
  // src/modules/ui/editorState.js
  function initInitialState({ editor: editor2 }) {
    if (!editor2) return;
    if (!editor2.value.trim()) {
      editor2.value = "# SnapDock Start typing...";
    }
    editor2.focus();
  }

  // src/modules/ui/theme.js
  var THEME_KEY = "snapdock_theme";
...
`;
  ...
  function initViewModeToggle({ toggleBtn, editor: editor2, preview: preview2 }) {
    if (!toggleBtn || !editor2 || !preview2) return;
    preview2.classList.add("hidden");
    editor2.style.display = "block";
    toggleBtn.textContent = "Show Preview";
    const updatePreview = () => {
      preview2.innerHTML = renderMarkdown(editor2.value);
    };
    toggleBtn.addEventListener("click", () => {
      const showingPreview = !preview2.classList.contains("hidden");
      if (showingPreview) {
        preview2.classList.add("hidden");
        editor2.style.display = "block";
        toggleBtn.textContent = "Show Preview";
    ...
    editor2.addEventListener("input", () => {
      if (!preview2.classList.contains("hidden")) {
        updatePreview();
      }
    });
  }

  // src/modules/file/recent.js
  var RECENT_KEY = "snapdock_recent_files";
  ...
  function getDisplayName(filePath) {
    ...
    const base2 = parts[parts.length - 1];
    return base2.replace(/\.[^/.]+$/, "");
  }
  function renderRecentFiles(container, editor2) {
    ...
    recent.forEach((filePath) => {
      ...
      li.addEventListener("click", async () => {
        ...
        if (content !== null) editor2.value = content;
  ...
  async function saveCurrentFile(editor2) {
    const success = await window.electronAPI.saveFile(editor2.value);
    if (success) alert("File saved successfully!");
  }
  async function loadContent(editor2) {
    const result = await window.electronAPI.openFile();
    if (result !== null) {
      editor2.value = result.content;
      return result;
    }
    return null;
  }
  ...
  var AUTOSAVE_KEY = "snapdock_autosave";
  function initAutosave({ editor: editor2 }) {
    if (!editor2) return;
    const saved = localStorage.getItem(AUTOSAVE_KEY);
    if (saved) editor2.value = saved;
    let timer;
    editor2.addEventListener("input", () => {
      clearTimeout(timer);
      timer = setTimeout(() => {
        localStorage.setItem(AUTOSAVE_KEY, editor2.value || "");
      }, 500);
    });
  }
  ...
  var editor = byId("markdownInputMain");
  var preview = byId("previewMain");
  var themeToggleBtn = byId("themeToggleBtnBottom");
  var previewToggleBtn = byId("previewToggleBtn");
  var versionTag = byId("versionTag");
  var recentList = byId("recentFilesList");
  var fileTreeList = byId("fileTreeList");
  ...
  async function setVersionTag() {
    ...
    const versionTag2 = document.getElementById("versionTag");
    versionTag2.textContent = `SnapDock ${info.version} (${info.build}) \u2014 ${info.date}`;
  }
  function setFilenameDisplay(filePath) {
    const parts = filePath.split(/[\\/]/);
    const base2 = parts[parts.length - 1];
    const name = base2.replace(/\.[^/.]+$/, "");
  ...
  document.getElementById("helpBtn")?.addEventListener("click", async () => {
    ...
    const md2 = window.markdown.render(content);
